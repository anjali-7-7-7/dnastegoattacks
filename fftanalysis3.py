...
# Screen at least 20 keys to get reliable rate estimate
MIN_SCREEN = 20
attempt = 0
while attempt < 50:
    ...
    attempt += 1
    # Keep going until minimum screened and both found
    if attempt >= MIN_SCREEN and vulnerable_key is not None and resistant_key is not None:
        break
...
if vulnerable_key is None:
    ...
    print(f"Note: using lowest p-value key as vulnerable proxy (p={best[1]:.4f})")

if resistant_key is None:
    # Use a key with highest p-value from all_results
    best = max(all_results, key=lambda x: x[1])
    np.random.seed(best[0] * 13)
    candidate = os.urandom(32)
    out = gen(candidate, nonce, N_BLOCKS, rounds=20)
    resistant_key = (candidate, best[0], best[1], out)
    print(f"Note: using highest p-value key as resistant proxy (p={best[1]:.4f})")
...
def compute_ac(data, max_lag=200, n=50000):
    ...
    lb_pvals = np.array(lb['lb_pvalue'].values, dtype=np.float64)
    sig_mask = lb_pvals < 0.05
